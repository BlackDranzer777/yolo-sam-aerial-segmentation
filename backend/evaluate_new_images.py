"""
evaluate_new_images.py — Batch evaluation of 10 new ICG images.

Runs the full YOLO+SAM pipeline on images not yet in the 12-image study,
computes per-class IoU/Precision/Recall, and saves results to
evaluation_new_10.json in this directory.

Run from the backend/ directory:
    python evaluate_new_images.py
"""
import sys
import os
import json
...
# ── Import pipeline & evaluator (must run from backend/) ──────────────────────
...
# 10 new images — not in the original 12-image study
# (001, 004, 011, 022, 075, 150, 165, 235, 238, 411, 531, 568 are already used)
NEW_IMAGE_IDS = ['002', '005', '016', '019', '021', '026', '028', '031', '040', '042']

OUTPUT_FILE = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'evaluation_new_10.json')
...
def run_batch():
    ...
    for img_id in NEW_IMAGE_IDS:
        ...
        if not os.path.exists(img_path):
            print(f'[SKIP] {img_id} — image file not found: {img_path}')
            all_results.append({'image_id': img_id, 'error': 'image not found'})
            continue

        gt_path = os.path.join(config.LABEL_MASKS_DIR, f'{img_id}.png')
        if not os.path.exists(gt_path):
            print(f'[SKIP] {img_id} — ground truth not found: {gt_path}')
            all_results.append({'image_id': img_id, 'error': 'ground truth not found'})
            continue
        ...
        try:
            ...
            print(f'  Overall IoU: {overall["mean_iou"]:.4f}  '
                  f'Precision: {overall["mean_precision"]:.4f}  '
                  f'Recall: {overall["mean_recall"]:.4f}')

            for obj in eval_out['per_object']:
                if obj['iou'] is not None:
                    print(f'    {obj["class_name"]:10s}  IoU={obj["iou"]:.4f}  '
                          f'P={obj["precision"]:.4f}  R={obj["recall"]:.4f}  '
                          f'({obj["note"]})')
                else:
    ...
    with open(OUTPUT_FILE, 'w', encoding='utf-8') as f:
        # masks are numpy arrays — convert booleans to lists for JSON
